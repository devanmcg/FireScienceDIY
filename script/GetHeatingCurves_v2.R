...
GetHeatingCurves <- function(FilePath, 
  ...
  if (t == 'h') {
  for(i in 1:length(tc_files)){ # Main single-channel logger work loop
  ...
  return(HeatingCurves)
    ...
    for(i in 1:length(tc_files)){ # loop through files (main work loop)
      ...
      message(Directions)
    for(j in 1:length(unique(rd$TC))) { # loop through multi-channel data on logger file
      rd_j <- filter(rd, TC == unique(rd$TC)[j])
